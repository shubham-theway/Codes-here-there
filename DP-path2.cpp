...
namespace path2
{
	...
	int dp(int **P, int **M, int k, int row, int col)
	{
		...
		if (row + 1 >=k || col - 1 < 0 || M[row +1][col - 1] == -1)
		{
			revdiag = dp(P, M, k, row +1, col - 1);
		}
		else
		{
			revdiag = M[row + 1][col - 1];
		}
		...
		if (next != -1 || diagonal != -1)
		{
			if (revdiag != -1)
			{
				if (revdiag + P[row][col] < total)
					total = revdiag + P[row][col];
			}
		}
		else if (revdiag != -1)
		{
